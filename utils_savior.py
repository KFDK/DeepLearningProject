...
class Storage():
	...
	def reset(self):
		...
		self.step = 0
		self.logits = torch.zeros(self.num_steps, self.num_envs, self.act_shape)


	def store(self, obs, action, reward, done, info, log_prob, value, logits):
		...
		self.logits[self.step] = logits.clone().detach()
	...
	def get_generator(self, batch_size=1024):
		...
		for indices in iterator:
			...
			logits = self.logits.reshape(-1, self.act_shape)[indices].cuda()
			yield obs, action, log_prob, value, returns, advantage, logits
